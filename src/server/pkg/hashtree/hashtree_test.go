...
func TestList(t *testing.T) {
	...
	expectedPaths := []string{"dir", "dir.bar", "dir2", "foo"}
	i := 0
...
func makeMergeReader() (*MergeReader, error) {
	expectedBuf := &bytes.Buffer{}
	w := NewWriter(expectedBuf)
	writeMergeNode(w, "/", "5d5e6bf6978265596cc1302f0bc368be893a4c14ce742b76dc20e983eac3446c7b022672c53ed6d80314b0c743fd22f50c07a09a79775165ce063ce984a91946", 8)
	writeMergeNode(w, "/dir-left", "9cf1225943e6797220168992de346051cdc762b9c1ba99bd6bdcc12ec3f1fea7", 1)
...
func TestMRWalk(t *testing.T) {
	mr, err := makeMergeReader()
	require.NoError(t, err)

	expectedPaths := []string{"/dir-shared", "/dir-shared/buzz-left", "/dir-shared/buzz-right", "/dir-shared/file-shared"}
	i := 0
	require.NoError(t, mr.Walk("/dir-shared", func(path string, node *NodeProto) error {
		require.Equal(t, expectedPaths[i], path)
		i++
		return nil
	}))
...
func TestMRGlob(t *testing.T) {
	mr, err := makeMergeReader()
	require.NoError(t, err)

	expectedPaths := []string{"/dir-left/bar-left", "/dir-right/bar-right", "/dir-shared/buzz-left", "/dir-shared/buzz-right", "/dir-shared/file-shared"}
	i := 0
	require.NoError(t, mr.Glob("/*/*", func(path string, node *NodeProto) error {
		require.Equal(t, expectedPaths[i], path)
		i++
		return nil
	}))
