...
` + codeend,
		Run: cmdutil.RunFixedArgs(0, func(args []string) error {
			...
			if err := client.ListJobF(pipelineName, commits, outputCommit, func(ji *ppsclient.JobInfo) error {
				pretty.PrintJobInfo(writer, ji, long)
				return nil
			}); err != nil {
				return err
			}
...
` + codeend,
		Run: cmdutil.Run(func(args []string) error {
			...
			writer := tabwriter.NewWriter(os.Stdout, pretty.JobHeader)
			for _, jobInfo := range jobInfos {
				pretty.PrintJobInfo(writer, jobInfo, long)
			}
...
`,
	...
	listPipeline := &cobra.Command{
		...
		Run: cmdutil.RunFixedArgs(0, func(args []string) error {
			...
			for _, pipelineInfo := range pipelineInfos {
				pretty.PrintPipelineInfo(writer, pipelineInfo, long)
			}
			return writer.Flush()
		}),
	}
	rawFlag(listPipeline)
	longFlag(listPipeline)
